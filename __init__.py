...
class DatePicker(BoxLayout):
    ...
    def __init__(self, **kw):
        super(DatePicker, self).__init__(**kw)
        self.init_roulettes()
    ...
    def set_selected_date(self, *args):
        new_dt = self.get_date()
        if new_dt:
            self.selected_date = new_dt

    def _bind_updates(self, *args):
        for field in self.date_fields:
            getattr(self, field).bind(selected_value=self.set_selected_date,
                                      in_motion=self._update_in_motion)
...
class TimePicker(BoxLayout):
    ...
    def __init__(self, **kw):
        super(TimePicker, self).__init__(**kw)
        self.init_roulettes()
    ...
    def init_roulettes(self):
        ...
        self.set_selected_time()
        self._bind_updates()

        children = [hour, Colon(), minute, Colon(), second]
        add = self.add_widget
        width = dp(20)
        for c in children:
            add(c)
    ...
    def set_selected_time(self, *args):
        new_dt = self.get_time()
        if new_dt:
            self.selected_time = new_dt

    def _bind_updates(self, *args):
        for field in self.time_fields:
            getattr(self, field).bind(selected_value=self.set_selected_time,
                                      in_motion=self._update_in_motion)
